...
public class MyAgent extends PacmanAgent {
    ...
    Suche suche;
    Knoten loesungsKonten;
    LinkedBlockingQueue<PacmanAction> pacmanActionList;
    PacmanTileType[][] view;
    boolean worldChanged = true;
...
/*        System.out.println("viewsize: " + view.length + "*" + view[0].length);
     ...
     * action() wählt die nächste(n) sinnvolle(n) Aktion(en), gemäß Regeln.
    ...
    public PacmanAction action() {
         ...
         * Dieser Part soll den Agenten so intelligent wie möglich handeln lassen.
         *
         * Zum Beispiel:
        ...
        if (worldChanged) {
            ...
            switch (suchstrategie){
                case TIEFENSUCHE:
                    suche = new Tiefensuche(this.percept, defineGoal(view));
                    break;
                case BREITENSUCHE:
                    suche = new Breitensuche(this.percept, defineGoal(view));
                    break;
                case DIJKSTAR:
                    suche = new Dijkstra(this.percept, defineGoal(view));
                    break;
                case BESTENSUCHE:
                    suche = new Bestensuche(this.percept, defineGoal(view));
                    break;
                case ASTERN:
                    suche = new AStern(this.percept, defineGoal(view));
                    break;
            }
            ...
            if (loesungsKonten != null){
                ...
                throw new RuntimeException("Kein Lösungsweg gefunden");
            }
     ...
     */
    public Knoten defineGoal(PacmanTileType[][] view){
        ...
        return new Knoten(zielview, null);
    }
}
